...
contract StatusOracle is Ownable {
    ...
    event SetMarketOpeningTime(
        uint256 oldMarketOpeningTime,
        uint256 newMarketOpeningTime
    );
    ...
    event SetAssetPauseTime(
        address asset,
        uint256 oldPauseTime,
        uint256 newPauseTime
    );
    ...
    modifier pauserAndOwner() {
        require(
            pauser == msg.sender || owner == msg.sender,
            "pauserAndOwner: caller is not owner or pauser!"
        );
        _;
    }
     ...
     */
    function _setMarketOpeningTime(
        uint256 _marketOpeningTime
    ) public onlyOwner isValidTime(_marketOpeningTime) {
    ...
    function _setDuration(
        uint256 _duration
    ) public onlyOwner isValidTime(_duration) {
    ...
    function _setDateBatch(
        uint256[] calldata _timestamps,
        uint256[] calldata _marketOpeningTimes,
        uint256[] calldata _marketclosingTimes
    ) external {
        require(
            _timestamps.length == _marketOpeningTimes.length &&
                _marketOpeningTimes.length == _marketclosingTimes.length,
        ...
        for (uint256 i = 0; i < _timestamps.length; i++) {
            _setDate(
                _timestamps[i],
                _marketOpeningTimes[i],
                _marketclosingTimes[i]
            );
        }
    }
    ...
    function _unpauseAsset(address _asset) external onlyOwner {
        ...
        (_oldAssetPauseTime, assetPauseTime[_asset]) = (
            assetPauseTime[_asset],
            0
        );
    ...
    function _getAssetStatus(
        ...
        // Filter weekend
        if (_secondsPassedInWeek / 1 days > 4) return false;

        (uint256 _marketOpeningTime, uint256 _marketclosingTime) = (
            marketOpeningTime,
            marketOpeningTime + duration
        );
        ...
        if (_date.activated)
            (_marketOpeningTime, _marketclosingTime) = (
                _date.marketOpeningTime,
                _date.marketclosingTime
            );
        ...
        if (
            _secondsPassedInDay < _marketOpeningTime ||
            _secondsPassedInDay > _marketclosingTime
        ) return false;

        uint256 _pauseTime = assetPauseTime[_asset] > pauseTime
            ? assetPauseTime[_asset]
            : pauseTime;
    ...
    function getDateInfo(
        uint256 _timestamp
    ) external view returns (uint256, bool, uint256, uint256) {
