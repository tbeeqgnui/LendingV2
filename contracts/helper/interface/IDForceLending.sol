...
interface IInterestRateModelHelper is IInterestRateModel {
...
interface IControllerHelper {
    ...
    function getEnteredMarkets(
        address _account
    ) external view returns (IiTokenHelper[] memory);
...
interface IRewardDistributorHelper {
    ...
    function distributionSpeed(
        IiTokenHelper _iToken
    ) external view returns (uint256);

    function distributionSupplySpeed(
        IiTokenHelper _iToken
    ) external view returns (uint256);
