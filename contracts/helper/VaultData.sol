...
contract VaultData is Ownable {
    ...
    constructor(IiTokenHelper _priceToken) public {
        initialize(_priceToken);
    }

    function initialize(IiTokenHelper _priceToken) public {
    ...
    function getDistributionSupplyApy(
        ...
        return
            _rewardDistributor
                ...
                .mul(_asset.interestRateModel().blocksPerYear())
                .mul(
                    _priceOracle.getUnderlyingPrice(
                        _rewardDistributor.rewardToken()
                    )
                )
                .rdiv(_assetValue);
    }
