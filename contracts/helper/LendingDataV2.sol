...
contract LendingDataV2 is Ownable {
    ...
    function getAccountTotalValue(
        ...
        (, , _var.sumCollateral, _var.sumBorrowed) = calcAccountEquity(
            _account
        );
        return (
            _var.supplyValue,
    ...
    function getSupplyTokenData(
        ...
        if (_asset.isiToken()) {
            uint256 _interestPerDay = _supplyRatePerUnit(_asset) *
                _unitsPerDay(_asset);
            return (
    ...
    function getAccountBorrowInfo(
        ...
        return (
            ...
            getBalance(_asset, _account).mul(_var.assetPrice).div(
                _var.USDPrice
            ),
    ...
    function calcAccountEquity(
        ...
        for (uint256 i = 0; i < _var.collateralITokens.length; i++) {
            ...
            _var.sumCollateral = _var.sumCollateral.add(
                _var
                    ...
                    .balanceOf(_account)
                    .mul(
                        controller.priceOracle().getUnderlyingPrice(
                            _var.collateralITokens[i]
                        )
                    )
        ...
        for (uint256 i = 0; i < _var.borrowedITokens.length; i++) {
            (, _var.borrowFactor, , , , , ) = controller.markets(
    ...
    function getAccountAvailable(address _account) public view returns (bool) {
        IiToken[] memory _collateralITokens = controller.getEnteredMarkets(
            _account
        );
        for (uint256 i = 0; i < _collateralITokens.length; i++) {
            if (
                !controller.priceOracle().getAssetPriceStatus(
                    _collateralITokens[i]
                )
            ) return false;
        }
        IiToken[] memory _borrowedITokens = controller.getBorrowedAssets(
            _account
        );
