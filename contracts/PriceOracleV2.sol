...
contract Exponential is CarefulMath {
    ...
    function getExp(
        uint256 num,
        uint256 denom
    ) internal pure returns (Error, Exp memory) {
...
interface ExchangeRateModel {
    ...
    function getFixedInterestRate(
        uint256 interval
    ) external view returns (uint256);

    function getFixedExchangeRate(
        uint256 interval
    ) external view returns (uint256);
}
...
contract PriceOracleV2 is Exponential {
     ...
     */
    function _setPendingAnchor(
        address _asset,
        uint256 _newScaledPrice
    ...
    function _setPendingAnchorAdmin(
        address _newPendingAnchorAdmin
    ) external returns (uint256) {
        // Check caller = anchorAdmin.
        if (msg.sender != anchorAdmin) {
            return
    ...
    function setExchangeRate(
        ...
        uint256 _currentExchangeRate = ExchangeRateModel(_exchangeRateModel)
            .getExchangeRate();
        require(
        ...
        uint256 _maxSwingRate = ExchangeRateModel(_exchangeRateModel)
            .getMaxSwingRate(_maxSwingDuration);
        require(
            _maxSwingRate > 0 &&
                _maxSwingRate <=
    ...
    function _disableExchangeRate(address _asset) public returns (uint256) {
        ...
        emit SetExchangeRate(_asset, address(0), 0, 0, 0);
        return uint256(OracleError.NO_ERROR);
    }
    ...
    function setMaxSwingRate(
        ...
        ExchangeRateModel _exchangeRateModel = ExchangeRateModel(
            exchangeRates[_asset].exchangeRateModel
        );
        uint256 _newMaxSwingRate = _exchangeRateModel.getMaxSwingRate(
            _maxSwingDuration
        );
     ...
     * @return uint 0=success, otherwise a failure.
     */
    function _setAggregatorProxy(
        IAggregatorProxy _aggregatorProxy
    ) public returns (uint256) {
        // Check caller = anchorAdmin
        if (msg.sender != anchorAdmin) {
            return
        ...
        emit SetAggregatorProxy(
            address(_oldAssetAggregator),
     ...
     * @return uint 0=success, otherwise a failure.
     */
    function _disableAssetAggregator(address _asset) public returns (uint256) {
        // Check caller = anchorAdmin
        if (msg.sender != anchorAdmin) {
            return
                failOracle(
     ...
     */
    function _setAssetStatusOracle(
        address _asset,
        IStatusOracle _statusOracle
    ) public returns (uint256) {
        // Check caller = anchorAdmin
        if (msg.sender != anchorAdmin) {
            return
     ...
     * @return uint 0=success, otherwise a failure.
     */
    function _disableAssetStatusOracle(
        address _asset
    ) public returns (uint256) {
        // Check caller = anchorAdmin
        if (msg.sender != anchorAdmin) {
    ...
    function _disableAssetStatusOracleBatch(
        address[] calldata _assets
    ) external {
        for (uint256 i = 0; i < _assets.length; i++)
     ...
     */
    function _getAssetAggregatorPrice(
        address _asset
    ) internal returns (uint256) {
        address _assetAggregator = aggregator[_asset];
        if (_assetAggregator == address(0)) return 0;

        IAggregatorProxy _aggregatorProxy = aggregatorProxy;
        if (address(_aggregatorProxy) == address(0)) return 0;

        (uint256 _aggregatorPrice, uint8 _decimals) = _aggregatorProxy
            .getAggregatorData(_asset, IAggregator(_assetAggregator));
    ...
    function getAssetAggregatorPrice(
        address _asset
    ) external returns (uint256) {
    ...
    function _getAssetPrice(address _asset) internal returns (uint256) {
        ...
        if (_assetPrice == 0) return _getReaderPrice(_asset);
    ...
    function _getReaderPrice(address _asset) internal view returns (uint256) {
        ...
        return
            srcDiv(readerPrice, pow(10, uint256(_reader.decimalsDifference)));
    }
    ...
    function assetPrices(address _asset) internal returns (uint256) {
        ...
        if (paused) {
            ...
            if (_exchangeRateInfo.exchangeRateModel != address(0)) {
                uint256 _scale = ExchangeRateModel(
                    _exchangeRateInfo.exchangeRateModel
                ).scale();
                uint256 _currentExchangeRate = ExchangeRateModel(
                    _exchangeRateInfo.exchangeRateModel
                ).getExchangeRate();
    ...
    function _getAssetPriceStatus(address _asset) internal view returns (bool) {
        ...
        if (_statusOracle == IStatusOracle(0)) return true;
    ...
    function setPrice(
        address _asset,
        uint256 _requestedPriceMantissa
    ) external returns (uint256) {
    ...
    function setPriceStorageInternal(
        address _asset,
    ...
    ) internal pure returns (Error, bool, Exp memory) {
    ...
    function setPrices(
        ...
        for (uint256 i = 0; i < numAssets; i++) {
            result[i] = setPriceInternal(
                _assets[i],
                _requestedPriceMantissas[i]
            );
        }
