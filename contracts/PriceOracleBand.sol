...
contract PriceOracleBand is Exponential {
     ...
     * @return uint 0=success, otherwise a failure.
     */
    function _setAggregator(IAggregator _aggregator) public returns (uint256) {
        // Check caller = anchorAdmin
        if (msg.sender != anchorAdmin) {
            return
                failOracle(
     ...
     * @return uint 0=success, otherwise a failure.
     */
    function _disableAggregator() public returns (uint256) {
        // Check caller = anchorAdmin
        if (msg.sender != anchorAdmin) {
            return
                failOracle(
     ...
     * @return uint 0=success, otherwise a failure.
     */
    function _disablePriceModel(address _asset) public returns (uint256) {
        // Check caller = anchorAdmin
        if (msg.sender != anchorAdmin) {
            return
                failOracle(
    ...
    function _getAssetAggregatorPrice(
        ...
        if (aggregator == IAggregator(0)) return 0;

        string memory _assetSymbol = symbols[_asset];
        if (bytes(_assetSymbol).length == 0) return 0;

        (uint256 _aggregatorPrice, , ) = aggregator.getReferenceData(
            _assetSymbol,
            "USD"
        );
     ...
     * @return Uint mantissa of asset price (scaled by 1e18) or zero if unset or under unexpected case.
     */
    function _getPriceModelPrice(
        address _asset
    ) internal view returns (uint256) {
        IPriceModel _priceModel = priceModels[_asset];
        if (_priceModel == IPriceModel(0)) return 0;

        return _priceModel.getAssetPrice(_asset);
    }

    function getPriceModelPrice(
        address _asset
    ) external view returns (uint256) {
    ...
    function _getAssetPrice(address _asset) internal view returns (uint256) {
        ...
        if (_assetPrice == 0) _assetPrice = _getPriceModelPrice(_asset);
