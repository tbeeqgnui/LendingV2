...
contract FlashloanTest {
    ...
    function executeOperation(
        ...
        if (_data.length > 0) {
            ...
            // (, shortfall, ,) = IController(ILendingData(_lendingData).controller()).calcAccountEquity(_borrower);
            ...
            (, uint256 _actualRepay, ) = ILendingData(_lendingData)
                .getLiquidationInfo(
                    _borrower,
                    address(this),
                    _assetBorrowed,
    ...
    function decode(
        bytes memory _data
    ) public pure returns (address x, address y, address z, address v) {
        _data;
        assembly {
            x := mload(0x94)
            y := mload(0xa8)
