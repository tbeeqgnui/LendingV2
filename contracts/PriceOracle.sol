...
contract PriceOracle is Exponential {
    ...
    function _getAssetAggregatorPrice(
        ...
        if (address(_assetAggregator) == address(0)) return 0;

        (, int256 _aggregatorPrice, , , ) = _assetAggregator.latestRoundData();
        if (_aggregatorPrice <= 0) return 0;
                    ...
                    (
                        srcSub(
                            ...
                            srcAdd(
                                ...
                                uint256(_assetAggregator.decimals())
                            )
                        )
                    )
            );
