...
contract MSDController is Initializable, Ownable {
    ...
    ) public onlyOwner onlyMSD(_token) {
    ...
    function calcEquity(
        address _token
    ) public onlyMSD(_token) returns (uint256, uint256) {
        // Call `updateInterest()` on all minters to get the latest token data
        EnumerableSetUpgradeable.AddressSet storage _msdMinters = msdMinters[
            _token
        ];
     ...
     */
    function getMSDMinters(
        address _token
    ) public view returns (address[] memory _minters) {
        EnumerableSetUpgradeable.AddressSet storage _msdMinters = msdMinters[
            _token
        ];
