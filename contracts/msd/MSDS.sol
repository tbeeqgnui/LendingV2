 ...
 * @author dForce
 */
contract MSDS is Initializable, ReentrancyGuard, Ownable, ERC20, InterestUnit {
    ...
    function _updateInterest() internal {
        ...
        if (_getInterestUnit() != getAccrualInterestUnit()) {
            ...
            if (totalSupply != 0) {
                ...
                uint256 _unitDelta = _getInterestUnit().sub(
                    getAccrualInterestUnit()
                );

                // Gets the current supply interest rate.
                uint256 _interestRate = interestRateModel.getSupplyRate(
                    0,
                    0,
                    0,
                    0
                 ...
                 *  simpleInterestFactor = interestRate * unitDelta
                 *  exchangeRate = exchangeRate + simpleInterestFactor * exchangeRate
                 */
                uint256 _simpleInterestFactor = _interestRate.mul(_unitDelta);
                uint256 _exchangeRateInc = exchangeRate.rmul(
                ...
                // emit UpdateInterest(_getInterestUnit(), exchangeRate);
            }
     ...
     */
    function balanceOfUnderlyingStored(
        address _account
     ...
     */
    function balanceOfUnderlyingCurrent(
        address _account
    ...
    /**
     * @dev Returns the current per-unit(block/second) saving interest rate.
     */
    function supplyRatePerUnit() public view returns (uint256) {
    ...
    /**
     * @notice Interface for MSDController to track mint cap, simply returns 0.
     */
    function totalMint() external pure returns (uint256) {
        return 0;
    }
}
