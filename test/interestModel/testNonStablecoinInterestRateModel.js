...
describe("nonstablecoin interest rate model", async function () {
    ...
    return factor
      .mul(
        ur
          ...
          .mul(primaryFactor)
          .div(BASE.pow(1))
          .add(ur.pow(4).mul(40).div(BASE.pow(3)))
          .add(ur.pow(7).mul(25).div(BASE.pow(6)))
      )
      .div(1000);
  }
  ...
  describe("check borrow rate", function () {
    it("init", async function () {
      await init();
      let threshold = "0.05";
      nonstablecoinInterest = await deployNonStablecoinInterestRateModel(
        threshold
      );
    ...
    for (let i = 0; i <= 100; i++) {
      it(`when ur is ${i}%`, async function () {
        ...
        let cash = 100 - i;
        let reserves = 0;
        let borrowRate = await nonstablecoinInterest.getBorrowRate(
          cash,
          borrows,
          reserves
        );
        ...
        let delta = 0.00001;
        verifyAllowError(borrowAPR, calcResultAPR, delta);
      });
    }
  });
