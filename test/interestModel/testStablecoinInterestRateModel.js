...
const { deployStablecoinInterestRateModel } = require("../helpers/fixtures.js");
...
const { parseTokenAmount, verifyAllowError } = require("../helpers/utils.js");
...
describe("stablecoin interest rate model", async function () {
    ...
    return factor
      .mul(
        ur
          .pow(2)
          .div(BASE.pow(1))
          .add(ur.pow(4).div(BASE.pow(3)))
          .add(ur.pow(7).div(BASE.pow(6)))
          .add(ur.pow(32).div(BASE.pow(31)).mul(2))
      )
      .div(100)
      .div(blocksPerYear);
  }
  ...
  describe("check borrow rate", function () {
    it("init", async function () {
      await init();
      stablecoinInterest = await deployStablecoinInterestRateModel();
    ...
    for (let i = 0; i <= 100; i++) {
      it(`when ur is ${i}%`, async function () {
        ...
        let cash = 100 - i;
        let reserves = 0;
        let borrowRate = await stablecoinInterest.getBorrowRate(
          cash,
          borrows,
          reserves
        );
  ...
  describe("check supply rate", function () {
    for (let i = 0; i <= 100; i++) {
      it(`when ur is ${i}%`, async function () {
        ...
        await USDx.connect(users[0]).approve(
          iUSDx.address,
          await parseTokenAmount(iUSDx, "99999")
        );
        await USDT.connect(users[0]).approve(
          iUSDx.address,
          await parseTokenAmount(iUSDT, "99999")
        );
        ...
        await iUSDx._setNewReserveRatio(
          await parseTokenAmount(iUSDx, reserveRatio)
        );
        ...
        let supplyRate = await iUSDx.supplyRatePerUnit();
        let actualSupplyRate = supplyRate.mul(
          ethers.BigNumber.from(blocksPerYear.toString())
        );
        // console.log("supplyRate ", actualSupplyRate.toString());
        let calcSupplyRate = borrowRate
          .mul(ethers.BigNumber.from(blocksPerYear.toString()))
          .mul(ethers.BigNumber.from((i * 10 ** 18).toString()))
          .mul(
            ethers.BigNumber.from(((1 - reserveRatio) * 10 ** 18).toString())
          )
          .div(BASE)
          .div(BASE)
          .div(100);
        ...
        let delta = 0.00001;
        verifyAllowError(actualSupplyRate, calcSupplyRate, delta);
      });
    }
  });
});
