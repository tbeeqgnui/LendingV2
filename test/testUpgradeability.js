...
describe("Upgradeability", function () {
  let controller, iUSDx, iUSDT, iWBTC, iETH, owner, newOwner, rewardDistributor;
  ...
  it("Should be able to update controller implementation", async function () {
    const ControllerV2 = await ethers.getContractFactory("ControllerStock");
  ...
  it("Should be able to update iToken implementation", async function () {
    ...
    const tokenName = await iUSDx.name();
    const totalSupply = await iUSDx.totalSupply();

    const iTokenV2 = await upgradeProxy(iUSDx.address, ITokenV2, {
      unsafeAllowCustomTypes: true,
    });
  ...
  it("Should be able to change the owner of ProxyAdmin", async function () {
    ...
    await expect(
      upgradeProxy(iUSDx.address, ITokenV2, {
        unsafeAllowCustomTypes: true,
      })
    ).to.be.reverted;
    ...
    const iTokenV1 = await proxyAdmin
      .connect(newOwner)
      .getProxyImplementation(iUSDx.address);

    // New owner should be able to update
    await proxyAdmin.connect(newOwner).upgrade(iUSDx.address, iTokenV2.address);
  ...
  it("Should be able to change the admin of proxy", async function () {
    ...
    let newOwnerAddr = await newOwner.getAddress();
    const proxyAdmin = await getProxyAdmin();
    await proxyAdmin.changeProxyAdmin(iUSDx.address, newOwnerAddr);
    ...
    const proxy = await Proxy.attach(iUSDx.address);
