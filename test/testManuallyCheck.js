...
let delta;
let USX, iMUSX, USXS, EUX, iMEUX, EUXS, iUSX;
let msdController;
...
describe("Manual Examination", function () {
  ...
  describe("Manual examination:", function () {
    ...
    it("77. Supply wbtc to USX should fail due to does not enter market", async function () {
      await init();
    ...
    it("78. Supply 1w USDC and the borrow USX and EUX", async function () {
      // user1 supplies 1w USDT
      let supplyAmount = await parseTokenAmount(iUSDT, 10000);
      let borrowAmount = await parseTokenAmount(USX, 100);
    ...
    it.skip("79. Uses wbtc as collateral to mint USX and EUX.", async function () {
      let borrowAmount = await parseTokenAmount(EUX, 100);
      await controller.connect(user1).enterMarkets([iWBTC.address]);
    ...
    it("81. Mint 500 USX to withdraw earns when borrow rate is greater than supply rate", async function () {
      await init();
      await controller.connect(user1).enterMarkets([iUSDT.address]);
      let supplyAmount = await parseTokenAmount(iUSDT, 10000);
      let borrowAmount = await parseTokenAmount(USX, 500);
      let saveAmount = await parseTokenAmount(USXS, 100);
      let withdrawAmount = await parseTokenAmount(USX, 1);
      let newBlocks = 1000;
      ...
      await fixedInterestRateModel._setSupplyRate(USXS.address, supplyRate);

      // user1 supplies USXS
      await USX.connect(user1).approve(USXS.address, MAX);
      await USXS.connect(user1).mint(user1.address, saveAmount);

      await increaseBlock(newBlocks);

      // Now MSDS does not update any earning or debt
      // let msdTokenDetails = await msdController.msdTokenData(USX.address);
      // let msdsEarnedAmount = msdTokenDetails.earning;

      // expect(msdsEarnedAmount).to.gt(0);

      //get extra USX to repay max
      await iUSDT.connect(user2).mint(user2.address, supplyAmount);
      await controller.connect(user2).enterMarkets([iUSDT.address]);
      await iMUSX.connect(user2).borrow(borrowAmount);
      await USX.connect(user2).transfer(user1.address, borrowAmount);
      // user1 repay iMUSX
      await USX.connect(user1).approve(iMUSX.address, MAX);
      await iMUSX.connect(user1).repayBorrow(MAX);

      // user1 withdraws USXS
      await USXS.connect(user1).redeem(user1.address, saveAmount);
      let USXSBalance = await USXS.balanceOf(user1.address);
      expect(USXSBalance).to.equal(0);

      await iMUSX.updateInterest();
      let reserve = await iMUSX.totalReserves();

      expect(reserve).to.gt("10000");

      await iMUSX._withdrawReserves(reserve);

      await iMUSX.updateInterest();
      reserve = await iMUSX.totalReserves();

      expect(reserve).to.lt(withdrawAmount);

      await expect(
        iMUSX._withdrawReserves(await parseTokenAmount(USX, 0.001))
      ).to.be.revertedWith("_withdrawReserves: Invalid withdraw amount!");
    });

    // Skipped as now iMSD reserves stores in iMSD, and MSDS currently is not in protocol
    it.skip("82. Mint 500 USX to withdraw earns when borrow interests is equal to supply interests", async function () {
      await init();
      await controller.connect(user1).enterMarkets([iUSDT.address]);
      let supplyAmount = await parseTokenAmount(iUSDT, 10000);
      let borrowAmount = await parseTokenAmount(USX, 500);
      ...
      await fixedInterestRateModel._setSupplyRate(USXS.address, "0");

      // update the interests
      await iMUSX.updateInterest();
      await USXS.updateInterest();
      ...
      await USX.connect(user1).approve(iMUSX.address, MAX);
      await iMUSX.connect(user1).repayBorrow(MAX);
      let iUSXBorrowedAmount = await iMUSX.callStatic.borrowBalanceCurrent(
        user1.address
      );
      expect(iUSXBorrowedAmount).to.equal(0);
    ...
    // Skipped as now iMSD reserves stores in iMSD, and MSDS is currently is not in protocol
    it.skip("83. Mint 500 USX but withdraw too much reserves", async function () {
      await init();
      await controller.connect(user1).enterMarkets([iUSDT.address]);
      let supplyAmount = await parseTokenAmount(iUSDT, 10000);
      let borrowAmount = await parseTokenAmount(USX, 500);
      ...
      // totalReserve will be current reserve plus 1 block accumulation
      const reserve = (await iMUSX.totalReserves()).add(
        rmul(await iMUSX.totalBorrows(), await iMUSX.borrowRatePerUnit())
      );

      await expect(
        iMUSX._withdrawReserves(
          reserve.add(10) // 1wei more than the estimated reserve
        )
      ).to.be.revertedWith("_withdrawReserves: Invalid withdraw amount!");
    });
    ...
    it("85. Borrow rate is 0, and supply rate is 0", async function () {
      ...
      expect(USXBalance).to.equal(borrowAmount);
    ...
    it("86. Borrow rate is greater than 0, and supply rate is 0", async function () {
      ...
      expect(iUSXBorrowedAmount).to.gt(0);
    ...
    it("88. Set liquidation incentive to 0", async function () {
      ...
      await iETH.connect(user1).mint(user1.address, {
        value: await parseTokenAmount(iETH, "20"),
      });
...
describe("Liquidation", function () {
  async function checkAccountValues(account, expected) {
    let userData = await lendingData.callStatic.getAccountTotalValueForTest(
      account
    );
    let userSupplyBalanceValue = userData[0];
    ...
    console.log(userSupplyBalanceValue.toString());
    console.log(userCollateralBalanceValue.toString());
    console.log(userBorrowBalanceValue.toString());
    console.log(userHealthyFactor.toString());

    const base = ethers.utils.parseEther("1");

    expect(userSupplyBalanceValue.div(base)).to.equal(expected[0]);
    expect(userCollateralBalanceValue.div(base)).to.equal(expected[1]);
    expect(userBorrowBalanceValue.div(base)).to.equal(expected[2]);
    expect(userHealthyFactor.mul(10000).div(base)).to.equal(
      expected[3] * 10000
    );
  }

  describe("Liquidation repay borrow max 50%", function () {
    beforeEach(async function () {
      ...
      [borrower, liquidator] = users;

      const iTokens = [iMUSX, iUSX, iUSDT, iWBTC, iETH];
      ...
      await setOraclePrices(oracle, iTokens, [1, 1, 1, 40000, 3000]);

      borrower.address = await borrower.getAddress();
      liquidator.address = await liquidator.getAddress();

      // Prepare the 10000 USX and transfer to borrower
      await controller.connect(liquidator).enterMarkets([iWBTC.address]);
      await iWBTC
        .connect(liquidator)
        .mint(liquidator.address, await parseTokenAmount(iWBTC, 100));
      await iMUSX
        .connect(liquidator)
        .borrow(await parseTokenAmount(iMUSX, 50000));
      await USX.connect(liquidator).transfer(
        borrower.address,
        await parseTokenAmount(USX, 30000)
      );
      await iETH.connect(liquidator).mint(liquidator.address, {
        value: await parseTokenAmount(iETH, 500),
      });
      ...
      await USX.connect(liquidator).approve(
        iMUSX.address,
        ethers.constants.MaxUint256
      );
      ...
      await iUSX
        .connect(borrower)
        .mint(borrower.address, await parseTokenAmount(iUSX, 10000));

      await iUSDT
        .connect(borrower)
        .mint(borrower.address, await parseTokenAmount(iUSDT, 10000));

      await iWBTC
        .connect(borrower)
        .mint(borrower.address, await parseTokenAmount(iWBTC, 1));

      await controller
        .connect(borrower)
        .enterMarkets([iUSDT.address, iWBTC.address]);

      await iETH.connect(borrower).borrow(await parseTokenAmount(iETH, 5));
      await iUSDT.connect(borrower).borrow(await parseTokenAmount(iUSDT, 5000));
      await iMUSX.connect(borrower).borrow(await parseTokenAmount(iMUSX, 5000));

      // Now drop the BTC price to cause a shortfall
      await setOraclePrices(oracle, iTokens, [1, 1, 1, 20000, 3000]);
    });

    it("01. Initial check", async function () {
      await checkAccountValues(borrower.address, [40000, 30000, 25000, 0.9599]);
    });

    it("02. Liquidate 1 ETH seize iUSDT", async function () {
      const action = {
        target: iETH,
        executor: liquidator,
      ...
      await checkAccountValues(borrower.address, [36790, 26790, 22000, 0.9741]);
    });

    it("03. Liquidate 2.5 ETH seize iUSDT", async function () {
      const action = {
        target: iETH,
        executor: liquidator,
        func: "liquidateBorrow",
      ...
      await checkAccountValues(borrower.address, [31975, 21975, 17500, 1.0045]);
    });

    it("04. Liquidate 1 ETH seize iUSX", async function () {
      const action = {
        target: iETH,
        executor: liquidator,
        func: "liquidateBorrow",
      ...
      await checkAccountValues(borrower.address, [36790, 30000, 22000, 1.0909]);
    });

    it("05. Liquidate 2500 USDT seize iUSDT", async function () {
      const action = {
        target: iUSDT,
        executor: liquidator,
        func: "liquidateBorrow",
        args: [
          borrower.address,
          await parseTokenAmount(iUSDT, "2500"),
          iUSDT.address,
        ],
      };
      ...
      await checkAccountValues(borrower.address, [37325, 27325, 22500, 0.9715]);
    });

    it("06. Liquidate 2500 USX seize iUSX", async function () {
      const action = {
        target: iMUSX,
        executor: liquidator,
        func: "liquidateBorrow",
        args: [
          borrower.address,
          await parseTokenAmount(iMUSX, "2500"),
          iUSX.address,
        ],
      };
      ...
      await checkAccountValues(borrower.address, [37325, 30000, 22500, 1.0666]);
    });
  });
  describe("Liquidation repay borrow max 100%", function () {});
});
...
describe("Transfer/TransferFrom", function () {
  ...
  beforeEach(async function () {
    ...
    from.address = await from.getAddress();
    to.address = await to.getAddress();

    await setOraclePrices(oracle, iTokens, [1, 1, 1, 20000, 3000]);

    // Prepare the 10000 USX and transfer to from
    await controller.connect(to).enterMarkets([iWBTC.address]);
    await iWBTC
      .connect(to)
      .mint(to.address, await parseTokenAmount(iWBTC, 100));
    await iMUSX.connect(to).borrow(await parseTokenAmount(iMUSX, 50000));
    await USX.connect(to).transfer(
      from.address,
      await parseTokenAmount(USX, 20000)
    );

    // Deposit ETH for borrow
    await iETH
      .connect(to)
      .mint(to.address, { value: await parseTokenAmount(iETH, 500) });

    // Supply USX:10,000；USDT:10,000；WBTC:1
    await USX.connect(from).approve(iMUSX.address, ethers.constants.MaxUint256);
    await USX.connect(from).approve(iUSX.address, ethers.constants.MaxUint256);
    await iUSX
      .connect(from)
      .mint(from.address, await parseTokenAmount(iUSX, 10000));

    await iUSDT
      .connect(from)
      .mint(from.address, await parseTokenAmount(iUSDT, 10000));

    await iWBTC
      .connect(from)
      .mint(from.address, await parseTokenAmount(iWBTC, 1));

    // Use iUSDT iWBTC as colllateral
    await controller.connect(from).enterMarkets([iUSDT.address, iWBTC.address]);

    // Borrow ETH:1；USDT:5000 iMUSX:5000
    await iETH.connect(from).borrow(await parseTokenAmount(iETH, 1));
    await iUSDT.connect(from).borrow(await parseTokenAmount(iUSDT, 5000));
    await iMUSX.connect(from).borrow(await parseTokenAmount(iMUSX, 5000));
  });

  it("01. Should be able to transfer all iUSX as it's not collateral", async function () {
    const action = {
      target: iUSX,
      executor: from,
      func: "transfer",
      args: [to.address, await iUSX.balanceOf(from.address)],
    };

    await executeAndVerify(action, [iUSX], [from.address, to.address]);
  });

  it("02. Should be able to transfer all iUSDT as there is enough equity", async function () {
    const action = {
      target: iUSDT,
      executor: from,
      func: "transfer",
      args: [to.address, await iUSDT.balanceOf(from.address)],
    };

    await executeAndVerify(action, [iUSDT], [from.address, to.address]);
  });

  it("03. Should be able to transfer all iWBTC when there is no borrow", async function () {
    // Repay all borrowed assets
    await iETH
      .connect(from)
      .repayBorrow({ value: ethers.utils.parseEther("2") });
    await iUSDT.connect(from).repayBorrow(ethers.constants.MaxUint256);
    await iMUSX.connect(from).repayBorrow(ethers.constants.MaxUint256);

    const borrowedAssets = await controller.getBorrowedAssets(from.address);
    expect(borrowedAssets.length).to.eq(0);

    const action = {
      target: iWBTC,
      executor: from,
      func: "transfer",
      args: [to.address, await iWBTC.balanceOf(from.address)],
    };

    await executeAndVerify(action, [iWBTC], [from.address, to.address]);
  });

  describe("Check Equity for transfer", async function () {
    let maxRedeemable;

    beforeEach(async function () {
      await controller.connect(from).enterMarkets([iUSX.address]);
      await iETH.connect(from).borrow(await parseTokenAmount(iETH, 4));
    });

    it("04. Should be able to transfer iUSX up to maxRedeemable", async function () {
      ...
      const action = {
        ...
        executor: from,
        func: "transfer",
        args: [to.address, maxRedeemable],
      };

      await executeAndVerify(action, [iUSX], [from.address, to.address]);
    });

    it("05. Should be able to transfer iUSDT up to maxRedeemable", async function () {
      ({ maxRedeemable } = await getMaxRedeemable(
        controller,
        oracle,
        from.address,
    ...
    it("06. Should be able to transfer iWBTC up to maxRedeemable", async function () {
      ({ maxRedeemable } = await getMaxRedeemable(
        controller,
        oracle,
        from.address,
    ...
    it("07. Should not be able to transfer iUSX more than maxRedeemable", async function () {
      ({ maxRedeemable } = await getMaxRedeemable(
        controller,
        oracle,
        from.address,
      ...
      await expect(
        iUSX.connect(from).transfer(to.address, maxRedeemable.add(1))
      ).to.revertedWith("Account has some shortfall");
    });

    it("08. Should not be able to transfer iUSDT more than maxRedeemable", async function () {
      ({ maxRedeemable } = await getMaxRedeemable(
        controller,
        oracle,
        from.address,
      ...
      await expect(
        iUSDT.connect(from).transfer(to.address, maxRedeemable.add(1))
      ).to.revertedWith("Account has some shortfall");
    });

    it("09. Should not be able to transfer iWBTC more than maxRedeemable", async function () {
      ({ maxRedeemable } = await getMaxRedeemable(
        controller,
        oracle,
        from.address,
      ...
      await expect(
        iWBTC.connect(from).transfer(to.address, maxRedeemable.add(1))
      ).to.revertedWith("Account has some shortfall");
    });

    it("10. Should not be able to transferFrom iUSX/iUSDT/iWBTC as the allowance is 0", async function () {
      await expect(
        iUSX.connect(to).transferFrom(from.address, to.address, 1)
      ).to.revertedWith("SafeMath: subtraction overflow");

      await expect(
        iUSDT.connect(to).transferFrom(from.address, to.address, 1)
      ).to.revertedWith("SafeMath: subtraction overflow");

      await expect(
        iWBTC.connect(to).transferFrom(from.address, to.address, 1)
      ).to.revertedWith("SafeMath: subtraction overflow");
    });

    it("11. Should be able to transferFrom iUSX up to the allowance < maxRedeemable", async function () {
      const allowance = await parseTokenAmount(iUSX, 1000);
      await iUSX.connect(from).approve(to.address, allowance);
      const action = {
        target: iUSX,
        executor: to,
        func: "transferFrom",
        args: [from.address, to.address, allowance],
      };

      await executeAndVerify(action, [iUSX], [from.address, to.address]);
    });

    it("12. Should not be able to transferFrom iUSX more than the allowance", async function () {
      const allowance = await parseTokenAmount(iUSX, 1000);
      await iUSX.connect(from).approve(to.address, allowance);
      ...
      await expect(
        iUSX
          .connect(to)
          .transferFrom(from.address, to.address, allowance.add(1))
      ).to.revertedWith("SafeMath: subtraction overflow");
    });

    it("13. Should be able to transferFrom iUSX up to the maxRedeemable < allowance", async function () {
      const allowance = await parseTokenAmount(iUSX, 9000);
      await iUSX.connect(from).approve(to.address, allowance);
      ...
      expect(maxRedeemable).to.lt(allowance);

      const action = {
        target: iUSX,
        executor: to,
        func: "transferFrom",
        args: [from.address, to.address, maxRedeemable],
      };

      await executeAndVerify(action, [iUSX], [from.address, to.address]);
    });

    it("14. Should not be able to transferFrom iUSX more than the maxRedeemable < allowance", async function () {
      const allowance = await parseTokenAmount(iUSX, 9000);
      await iUSX.connect(from).approve(to.address, allowance);
      ...
      // maxRedeemable is about 8750
      expect(maxRedeemable).to.lt(allowance);

      await expect(
        iUSX
          .connect(to)
          .transferFrom(from.address, to.address, maxRedeemable.add(1))
      ).to.revertedWith("Account has some shortfall");
    });
  });

  describe("Transfer/TransferFrom when have shortfall", async function () {
    beforeEach(async function () {
      // Drop the price to borrow
      await setOraclePrices(oracle, [iETH], [300]);

      await iETH.connect(from).borrow(await parseTokenAmount(iETH, 4));

      await setOraclePrices(oracle, [iETH], [3000]);
    });

    it("15. Should not be able to transfer/transferFrom iUSDT as there is a shortfall", async function () {
      await expect(iUSDT.connect(from).transfer(to.address, 0)).to.revertedWith(
        "Account has some shortfall"
      );

      const allowance = await parseTokenAmount(iUSDT, 9000);
      await iUSDT.connect(from).approve(to.address, allowance);
      await expect(
        iUSDT.connect(to).transferFrom(from.address, to.address, 0)
      ).to.revertedWith("Account has some shortfall");
    });
  });
});
