...
async function setETHBalance(address, balance) {
  //await helpers.setBalance(userLiquidator.address, depositAmount.mul(2));
...
async function verifyNonlistediToken(
  controller,
  func,
  remainningArgs,
  iTokenArgIndex = 0
) {
  const nonListediToken = controller.address;
  let args;

  args = [
    ...remainningArgs.slice(0, iTokenArgIndex),
    nonListediToken,
    ...remainningArgs.slice(iTokenArgIndex),
  ];

  await expect(controller[func](...args)).to.be.revertedWith(
...
async function myDeployProxy(
  ...
  if (!implementationAddress) {
    ...
    tx = await impl[params.initializer](...args);
    await txWait(network.name, tx);
  } else {
    impl = { address: implementationAddress };
  }
...
async function myUpgradeProxy(proxyAddress, contractFactory, params) {
  const admin = await getProxyAdmin();

  const nextImpl = params.constructorArgs
    ? await contractFactory.deploy(...params.constructorArgs)
...
async function msdEquityPerBlock(iMSD, MSDS) {
  let earning = rmul(await iMSD.totalBorrows(), await iMSD.borrowRatePerUnit());
  let debt = rmul(
    await MSDS.totalSupply(),
    rmul(await MSDS.exchangeRateStored(), await MSDS.supplyRatePerUnit())
  );
