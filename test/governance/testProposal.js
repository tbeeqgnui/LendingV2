...
let delta;
let USX, iMUSX, USXS, EUX, iMEUX, EUXS;
let msdController;
...
describe("Governance Proposal Examination", function () {
  ...
  async function init() {
    ...
    newController = await deployController();

    // Deploy a new interest rate model
    newInterestModelRate = await deployStablecoinInterestRateModel();
  }
  ...
  describe("Test iToken contract", function () {
    ...
    it("Set flashloan fee ratio and protocol fee ratio at the same time", async function () {
      ...
      let currentLiquidationIncentive =
      ...
      let currentBorrowSpeed =
        await rewardDistributor.globalDistributionSpeed();
      let currentSupplySpeed =
