...
let USDx, USDT, WBTC;
let USX, iMUSX, EUX, iMEUX, iUSX, iEUX;
let xTSLA, ixTSLA, xAAPL, ixAAPL;
let msdController;
let users, owner;
let lendingData;
let oracle;
...
describe.skip("Random test", function () {
  async function init() {
    ({
    ...
    ixAAPLDecimals = await ixAAPL.decimals();
    ...
    underlyings.push(USX);

    allTokens.push(iMEUX);
    underlyings.push(EUX);

    // iMSDs.push(iUSX);
    // iMSDsUnderlyings.push(USX);

    // iMSDs.push(iEUX);
    // iMSDsUnderlyings.push(EUX);

    allTokens.push(ixTSLA);
    underlyings.push(xTSLA);

    allTokens.push(ixAAPL);
    underlyings.push(xAAPL);
    ...
    await setOraclePrices(
      oracle,
      [iUSDx, iUSDT, iWBTC, ixTSLA, ixAAPL],
      [1, 1, 20000, 732, 132]
    );
    ...
    for (let i = 0; i < casesLength; i++) {
      it("Read test case and then run", async function () {
        ...
        if (allActions[randomAction] == "mint") {
          ...
          let userUnderlyingBalance =
            await iToken.callStatic.balanceOfUnderlying(user.address);
          console.log(
    ...
    it("repay and withdraw all", async function () {
          ...
          if (borrowedAmount.toString() > 0) {
            if (underlyingToken == zeroAddress) {
              ...
              if (userBalance.sub(repayAmount) > 0) {
                ...
                await iToken
                  .connect(users[16])
                  .repayBorrowBehalf(user.address, {
                    value: repayAmount,
                  });
              }
            } else {
      ...
      for (let j = 0; j < allTokens.length; j++) {
        ...
        if (isiToken) {
          ...
          let iMSDTotalReserves = await iToken.totalReserves();
