...
describe("iToken permit", async function () {
  ...
  describe("Test all scenarios for permit", function () {
    it("ITKN-PMIT-0: Domain Separator is correct", async function () {
      await init();
    ...
    it("ITKN-PMIT-1: Should permit correctly", async function () {
      await init();
    ...
    it("ITKN-PMIT-2: Should revert due to expired!", async function () {
      let currentNonce = await iToken.nonces(owner);
      const expiredTime = 1;
      const data = buildData(
        chainId,
    ...
    it("ITKN-PMIT-3: Should revert due to invalid signature", async function () {
      let currentNonce = await iToken.nonces(owner);
      const data = buildData(
        chainId,
        iToken.address,
