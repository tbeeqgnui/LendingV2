...
describe("iToken", function () {
  ...
  describe("Test the remaining queryable functions", function () {
    it("Query underlying balance", async function () {
      ...
      let user1USDTBalance = await iToken.callStatic.balanceOfUnderlying(
        user1.address
      );

      expect(user1USDTBalance).to.equal(
      ...
      user1USDTBalance = await iToken.callStatic.balanceOfUnderlying(
        user1.address
      );
    ...
    it("Query borrow rate block", async function () {
      ...
      let expectborrowRatePerUnit = ur
        .mul(interestPerBlock)
        .div(BASE)
        .add(baseInterestPerBlock);

      let borrowRatePerUnit = await iToken.borrowRatePerUnit();

      expect(borrowRatePerUnit).to.equal(expectborrowRatePerUnit);
    });
    ...
    it("Query supply rate block", async function () {
      ...
      supplyRate = await iToken.supplyRatePerUnit();
      expect(supplyRate).to.equal(0);
    });
  });

  describe("Test library ERC 20", async function () {
    it("ITKN-ERC-0: Should revert due to transfer from the zero address", async function () {
      await expect(
        underlying
          .connect(user2)
          .transferFrom(
    ...
    it("ITKN-ERC-1: Should revert due to burn from the zero address", async function () {
      await expect(
        underlying
          .connect(user2)
