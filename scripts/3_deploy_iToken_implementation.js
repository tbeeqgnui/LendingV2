...
const assetsToDeploy = ["eth", "usdc", "USX_msd"];
...
async function main() {
  ...
  if (!nonStableInterestModel) {
    console.log(
      "Do not get a valid non-stablecoin interest model contract address!"
    );
    return;
  }
  ...
  if (!stableInterestModel) {
    console.log(
      "Do not get a valid stablecoin interest model contract address!"
    );
    return;
  }
  ...
  for (index in assetsToDeploy) {
    ...
    let iToken = await deployiTokenContract(
      assetConfig,
      asset,
      controllerAddress,
      interestModelAddress,
