...
async function addToMarket(controllerContract, iToken, reserveConfig) {
  ...
  if (reserveConfig.interestModelType != "fixedInterestModel") {
    ...
    console.log(
      ...
      await formatToHumanReadable(tokenMarkets.collateralFactorMantissa)
    );
    console.log(
      "contract borrow factor is:     ",
      await formatToHumanReadable(tokenMarkets.borrowFactorMantissa)
    );
    console.log(
      "contract supply cap is:        ",
      await formatTokenAmount(iToken, tokenMarkets.supplyCapacity)
    );
    console.log(
      "contract borrow cacp is:       ",
      await formatTokenAmount(iToken, tokenMarkets.borrowCapacity),
      "\n"
    );
  }
}
...
async function main() {
  ...
  for (index in assetsToDeploy) {
    ...
    if (
      (tokenPriceSwing.toString() == 0 &&
        defaultMaxSwing.toString() != actualPriceSwing.toString()) ||
      tokenPriceSwing.toString() != actualPriceSwing.toString()
    ) {
  ...
  for (index in assetsToDeploy) {
    ...
    console.log(
      asset,
      "actualTokenPriceSwing",
      await formatToHumanReadable(actualTokenPriceSwing)
    );
  }
