...
async function deployInterestModelContract(modelType, interestModelAddress) {
  ...
  if (modelType == "nonStable") {
    if (!interestModelAddress) {
      ...
      let threshold = "0.05";
      interestModelContract = await deployNonStablecoinInterestRateModel(
        threshold
      );
    } else {
    ...
    if (!interestModelAddress) {
      ...
      interestModelContract = await deployStablecoinInterestRateModel();
    } else {
...
async function main() {
  ...
  modelType = "nonStable";
  let nonStableInterestModel = await deployInterestModelContract(
    modelType,
    commonConfigs.nonStableInterestModel
  );
  console.log(
    "Non-stable interest model contract is: ",
    nonStableInterestModel.address
  );

  // means this is a stable interest model contract.
  modelType = "stable";
  let stableInterestModel = await deployInterestModelContract(
    modelType,
    commonConfigs.stableInterestModel
  );
  console.log(
    "Stable interest model contract is: ",
    stableInterestModel.address
  );

  // means this is a fixed interest model contract.
  modelType = "fixed";
  let fixedInterestModel = await deployInterestModelContract(
    modelType,
    commonConfigs.fixedInterestModel
  );
