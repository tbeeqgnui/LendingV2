...
async function main() {
  ...
  let liquidationIncentive =
    allReservesConfigs[currentNet].liquidationIncentive;
  ...
  console.log(
    "contract close factor is: ",
    await formatToHumanReadable(contractCloseFactor, 2)
  );

  let contractLiquidationIncentive = await controller.liquidationIncentiveMantissa();
  console.log(
    "contract liquidation Incentive is: ",
    await formatToHumanReadable(contractLiquidationIncentive, 2)
  );
}
