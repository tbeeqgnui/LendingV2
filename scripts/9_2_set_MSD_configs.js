 ...
 * 1) Add USX tokens to MSD Controller's token list.
...
async function main() {
  ...
  let fixedInterestModelFactory = await ethers.getContractFactory(
    "FixedInterestRateModel"
  );
  let fixedInterestModel = fixedInterestModelFactory.attach(
    fixedInterestModelAddress
  );
  console.log(
    "fixed interest model contract address is: ",
    fixedInterestModel.address,
    "\n"
  );
  ...
  for (index in assetsToDeploy) {
    ...
    tx = await msdController._addMSD(msdTokenAddress, [
      imsdTokenAddress,
      msdsTokenAddress,
    ]);
    ...
    console.log(
      asset,
      " minters in MSD controller are: ",
      await msdController.getMSDMinters(msdTokenAddress),
      "\n"
    );
    ...
    let actualBorrowRate = ethers.utils
      .parseEther(assetConfig.borrowRate)
      .div(ethers.BigNumber.from(commonConfigs.blocksPerYear.toString()));
    if (currentBorrowRate.toString() != actualBorrowRate.toString()) {
      ...
      console.log(
        "iMSD to write borrow rate is: ",
        actualBorrowRate.toString()
      );
    ...
    let currentSupplyRate = await fixedInterestModel.supplyRatesPerBlock(
      msdsTokenAddress
    );
    let actualSupplyRate = ethers.utils
      .parseEther(assetConfig.supplyRate)
      .div(ethers.BigNumber.from(commonConfigs.blocksPerYear.toString()));
    if (currentSupplyRate.toString() != actualSupplyRate.toString()) {
      console.log("\nset suppply rate for iMSD token");
      console.log(
        "MSDS current supply rate is:  ",
        currentSupplyRate.toString()
      );
      console.log(
        "MSDS to write supply rate is: ",
        actualSupplyRate.toString()
      );
      tx = await fixedInterestModel._setSupplyRate(
        msdsTokenAddress,
        actualSupplyRate
      );
      await txWait(currentNet, tx);
      console.log(
        "after setting, MSDS current supply rate is: ",
        actualSupplyRate.toString()
      );
    }
  }
