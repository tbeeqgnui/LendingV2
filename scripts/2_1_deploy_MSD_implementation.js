...
const { deployMSD } = require("../test/helpers/fixtures.js");
...
async function main() {
  ...
  for (index in toDeployMSDTokens) {
    ...
    if (!contractAddresses.hasOwnProperty(msdToken)) {
      console.log(
        "\n Please set token address for underlying: ",
        msdToken,
        "\n"
      );
      continue;
    ...
    if (!msdTokenAddress) {
      ...
      let MSD = await deployMSD(
        assetConfig.msdTokenName,
        assetConfig.msdTokenSymbol,
        assetConfig.decimals,
        commonConfigs.msdImplementation
      );
