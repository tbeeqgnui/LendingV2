...
async function main() {
  try {
    ...
    let toDeployiTokens = ["iusdt"];
    let toDeployMSDTokens = ["iusdt_imsd"];
    ...
    let iTokenConfig = currentAssetConfigs["iusdt"];
    ...
    let iMSDConfig = currentAssetConfigs["iusdt_imsd"];
    ...
    // 6.1 Deploy aggregator contract
    const aggregatorContractName = "AggregatorProxy";
    const aggregatorArtifactsPath = `browser/artifacts/contracts/aggregatorModelV2/${aggregatorContractName}.sol/${aggregatorContractName}.json`;
    const aggregatorMetadata = JSON.parse(
      await remix.call("fileManager", "getFile", aggregatorArtifactsPath)
    );

    if (!currentContractAddresses.aggregatorProxy) {
      console.log("Going to deploy aggregator proxy contract!");
      const aggregatorProxyFactory = new ethers.ContractFactory(
        aggregatorMetadata.abi,
        aggregatorMetadata.bytecode,
        signer
      );
      const aggregatorProxyContract = await aggregatorProxyFactory.deploy();
      await aggregatorProxyContract.deployed();

      currentContractAddresses.aggregatorProxy = aggregatorProxyContract.address;
      console.log("Aggregator contract address is: ", currentContractAddresses.aggregatorProxy);
    }

    // Set aggregator in the oracle.
    let currentAggregator = await oracle.aggregatorProxy();
    console.log("current aggregatorProxy is: ", currentAggregator);
    if (currentAggregator != currentContractAddresses.aggregatorProxy) {
      console.log("Going to set aggregatorProxy in oracle contract!");
      tx = await oracle._setAggregatorProxy(currentContractAddresses.aggregatorProxy);
      await tx.wait(1);

      currentAggregator = await oracle.aggregatorProxy();
      console.log("after execution, aggregatorProxy is: ", currentAggregator);
    }

    // 6.2 Deploy iToken aggregator contract.
    const iTokenAggregatorContractName = "iTokenAggregatorModel";
    const iTokenAggregatorArtifactsPath = `browser/artifacts/contracts/aggregatorModelV2/${iTokenAggregatorContractName}.sol/${iTokenAggregatorContractName}.json`;
    const iTokenAggregatorMetadata = JSON.parse(
      await remix.call("fileManager", "getFile", iTokenAggregatorArtifactsPath)
    );

    if (!currentContractAddresses.iTokenAggregatorAddress) {
      console.log("Going to deploy iToken aggregator contract!");
      const iTokenAggregatorFactory = new ethers.ContractFactory(
        iTokenAggregatorMetadata.abi,
        iTokenAggregatorMetadata.bytecode,
        signer
      );
      const iTokenAggregatorContract = await iTokenAggregatorFactory.deploy();
      await iTokenAggregatorContract.deployed();

      currentContractAddresses.iTokenAggregatorAddress = iTokenAggregatorContract.address;
      console.log("iToken aggregator contract address is: ", currentContractAddresses.iTokenAggregatorAddress);
    }

    // TODO: Finally, use this way to set for many tokens
    // Set iToken aggregator in the oralce.
    // _setAssetAggregatorBatch
    let assets = [];
    let aggregators = [];
    let iTokenAggregatorAddress = currentContractAddresses.iTokenAggregatorAddress;
    for (let i = 0; i < toDeployiTokensLength; i++) {
      let currentiToken = toDeployiTokens[i];
      let iTokenProxyAddress = currentAssetConfigs[currentiToken]["iTokenAddress"];
      assets.push(iTokenProxyAddress);
      aggregators.push(iTokenAggregatorAddress);
    }

    // await oracle._setAssetAggregatorBatch(assets, aggregators);

    // 7.0 Deploy lending data implementation contract.
    const priceTokenAddress = currentAssetConfigs["iusdt_imsd"]["iMTokenAddress"];
