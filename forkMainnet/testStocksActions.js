...
let xTSLA, xAAPL, xAMZN, xCOIN;
let ixTSLA, ixAAPL, ixAMZN, ixCOIN;
let MSDs = [];
let iMSDs = [];
let iTokens = [];
let collateralMSDs = [];
...
let stockUSXAddress = mainnetContractAddresses["USX_sp"]["iToken"];
let stockEUXAddress = mainnetContractAddresses["EUX_sp"]["iToken"];

let xTSLAAddress = mainnetContractAddresses["xtsla_msd"]["token"];
let xAAPLAddress = mainnetContractAddresses["xAAPL_msd"]["token"];
...
let ixAAPLAddress = mainnetContractAddresses["xAAPL_msd"]["iToken"];
...
async function initContract() {
  ...
  xAAPL = await ethers.getContractAt("MSD", xAAPLAddress);
  ...
  ixAAPL = await ethers.getContractAt("iMSD", ixAAPLAddress);
...
describe("Simulate all actions in iMSD contract of stock pool", function () {
  ...
  it("Borrow and repay MSD token when market is open", async function () {
    ...
    for (let i = 0; i < alliMSDTokenLength; i++) {
      ...
      let beforeBorrowUserBorrowediTokenBalance =
        await iMSD.borrowBalanceStored(mocker);
      console.log(
  ...
  it("Repay borrow for others", async function () {
    ...
    for (let j = 0; j < alliMSDTokenLength; j++) {
      ...
      let beforeRepayBorrowUserBorrowediTokenBalance =
        await iMSD.borrowBalanceStored(mocker);
      console.log(
      ...
      let afterRepayBorrowUserBorrowediTokenBalance =
        await iMSD.borrowBalanceStored(mocker);
      console.log(
        "After repay borrow MSD, user borrowed balance is:   ",
