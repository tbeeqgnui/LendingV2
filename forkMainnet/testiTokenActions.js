...
let USXAddress = mainnetContractAddresses["USX_msd"]["token"];
let EURAddress = mainnetContractAddresses["EUX_msd"]["token"];
...
let iMUSXAddress = mainnetContractAddresses["USX_msd"]["iToken"];
let iMEURAddress = mainnetContractAddresses["EUX_msd"]["iToken"];
...
let iUSXAddress = mainnetContractAddresses["USX"]["iToken"];
let iEURAddress = mainnetContractAddresses["EUX"]["iToken"];
...
describe("Simulate all actions in iToken contract of general pool", function () {
  ...
  it("Supply and Redeem MSD token", async function () {
    ...
    for (let i = 0; i < alliMTokensLength; i++) {
      ...
      let beforeSuppliediTokenBalane = await iToken.balanceOf(
        contractExecutor._address
      );
      console.log(
        "Before supplying, user iToken balance is: ",
        beforeSuppliediTokenBalane.toString()
      );
      ...
      let beforeRedeemMSDBalance = await MSD.balanceOf(
        contractExecutor._address
      );
      console.log(
        "After supplying, user msd balance is:    ",
        beforeRedeemMSDBalance.toString()
      );

      let afterSuppliediTokenBalane = await iToken.balanceOf(
        contractExecutor._address
      );
      console.log(
        "After supplying, user iToken balance is: ",
        afterSuppliediTokenBalane.toString(),
        "\n"
      );
      ...
      console.log(
        "After Redeeming, user msd balance is:    ",
        afterRedeemMSDBalance.toString()
      );

      let afterRedeemiTokenBalane = await iToken.balanceOf(
        contractExecutor._address
      );
      console.log(
        "After redeeming, user iToken balance is: ",
        afterRedeemiTokenBalane.toString(),
        "\n"
      );
      ...
      console.log(
        "After Redeeming underlying, user msd balance is:    ",
        afterRedeemMSDBalance.toString()
      );

      afterRedeemiTokenBalane = await iToken.balanceOf(
        contractExecutor._address
      );
      console.log(
        "After redeeming underlying, user iToken balance is: ",
        afterRedeemiTokenBalane.toString(),
        "\n"
      );
    }
  ...
  it("Borrow and repay MSD token", async function () {
    ...
    for (let i = 0; i < alliTokensLength; i++) {
      ...
      let beforeBorrowUserBorrowediTokenBalance =
        await iToken.borrowBalanceStored(mocker);
      console.log(
      ...
      let afterBorrowUserBorrowediTokenBalance =
        await iToken.borrowBalanceStored(mocker);
      console.log(
      ...
      let afterRepayedUserBorrowediTokenBalance =
        await iToken.borrowBalanceStored(mocker);
      console.log(
  ...
  it("Repay borrow for others", async function () {
    ...
    for (let i = 0; i < alliMTokensLength; i++) {
      ...
      console.log(
        "After borrowing MSD, user balance is:                   ",
        afterUserMSDBalance.toString()
      );
      ...
      console.log(
        "After repaying MSD for others, liquidator balance is:   ",
        afterRepayUserMSDBalance.toString()
      );

      let afterRepayBorrowUserMSDBalance = await iToken.borrowBalanceStored(
        mocker
      );
      console.log(
        "After repaying MSD, user balance is:                    ",
        afterRepayBorrowUserMSDBalance.toString(),
        "\n"
      );
    }
  ...
  it("Liquidate borrow", async function () {
    ...
    console.log(
      "iDF collateral factor is: ",
      iDFDetails.collateralFactorMantissa.toString()
    );
    ...
    for (let i = 0; i < alliMTokensLength; i++) {
      ...
      let beforeUserMSDBorrowedBalance = await iToken.borrowBalanceStored(
        mocker
      );
      console.log(
      ...
      console.log(
        "Before liquidating MSD, liquidator iDF balance is: ",
        beforeiDFBalance.toString()
      );

      await iToken
        .connect(liquidator)
        .liquidateBorrow(mocker, liquidateAmount, iDF.address);

      let afterUserMSDBorrowedBalance = await iToken.borrowBalanceStored(
        mocker
      );
      console.log(
      ...
      console.log(
        "After liquidating MSD, liquidator iDF balance is:  ",
        afteriDFBalance.toString(),
        "\n"
      );
    }
    await stopImpersonatingAccount(repayer);
